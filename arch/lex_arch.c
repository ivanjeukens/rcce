#define yy_create_buffer arch_create_buffer
#define yy_delete_buffer arch_delete_buffer
#define yy_scan_buffer arch_scan_buffer
#define yy_scan_string arch_scan_string
#define yy_scan_bytes arch_scan_bytes
#define yy_flex_debug arch_flex_debug
#define yy_init_buffer arch_init_buffer
#define yy_flush_buffer arch_flush_buffer
#define yy_load_buffer_state arch_load_buffer_state
#define yy_switch_to_buffer arch_switch_to_buffer
#define yyin archin
#define yyleng archleng
#define yylex archlex
#define yyout archout
#define yyrestart archrestart
#define yytext archtext
#define yywrap archwrap
...
#define NR -1
#define RESERV_SIZE 107

const char RESERV_TABLE[][14] = {
	"abs",
	"access",
	"after",
	"alias",
...
const int TOKEN_TABLE[RESERV_SIZE] = {
	ABS,
	ACCESS,
	AFTER,
	ALIAS,
...
int i;
  ...
  for(i = 0;i < RESERV_SIZE;i++) {
    if(strcmp(key, RESERV_TABLE[i]) == 0) {
      return TOKEN_TABLE[i];
    }
    if(key[0] < RESERV_TABLE[i][0]) {
      return NR;
    }    
  }
...
#line 294 "arch.l"
{
	...
	itoken = Reserved(yytext);
	if(itoken == NR) {
	  archlval.text = (char *) malloc(sizeof(char)*(strlen(yytext)+1));
	  if(archlval.text == NULL) {
	    fprintf(stderr,"Out of memory.\n");
	    exit(1);
	  }
	  strcpy(archlval.text, yytext);
	  return(Identifier);
	}
	else {
	  return(itoken);
